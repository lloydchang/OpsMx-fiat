...
./gradlew --no-daemon -PenableCrossCompilerPlugin=true fiat-web:installDist -x test


   # Assigning Rhel Image Name according to Quay.io Details
   IMAGENAME="quay.io/opsmxpublic/ubi8-oes-fiat:${GITHASH}-${BUILD_NUMBER}"
   
   # Assigning Rhel Image Name according to Docker.io Details
   RELEASE_IMAGENAME="opsmx11/ubi8-oes-fiat:${GITHASH}-${BUILD_NUMBER}"
...
echo \"Fiat\": \"${IMAGENAME}\" > file.properties;
